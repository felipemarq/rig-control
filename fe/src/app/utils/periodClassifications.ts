...
const SCHEDULED_STOP = [
  {
    id: "UNBILLED_SCHEDULED_STOP",
    classification: "Parada de Manutenção não faturada",
  },
  {
    id: "BILLED_SCHEDULED_STOP",
    classification: "Parada de Manutenção faturada",
  },
];
...
export const getPeriodClassification = (parameter: string) => {
  ...
  if (parameter === "STAND_BY") {
    return periodClassifications.STAND_BY.map(({ id, classification }) => {
      return {
        value: id,
        label: classification,
      };
