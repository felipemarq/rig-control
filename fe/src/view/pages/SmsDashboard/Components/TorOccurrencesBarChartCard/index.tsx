...
export const TorOccurrencesBarChartCard = () => {
  ...
  const convertedResul = occurrencesTaxes?.torOccurrences?.map((occurrence) => {
    if (!occurrence) {
      return {
        month: 0,
        tax: 0,
  ...
  return (
    <Card className="">
      <CardHeader>
        <CardTitle>Taxa de Ocorrências Registradas (TOR) por Mês </CardTitle>
        <CardDescription>
          A TOR representa a taxa de ocorrências registradas, calculada com base no número
          de eventos por mês e nas horas trabalhadas.
        </CardDescription>
      </CardHeader>
      <CardContent>
