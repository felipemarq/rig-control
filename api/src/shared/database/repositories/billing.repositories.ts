...
export class BillingRepository {
  ...
  async findAll({ startDate, endDate }) {
    ...
    SUM(b.scheduled_stop_amount) AS scheduledStopAmount,
    SUM(b.unbilled_scheduled_stop_amount) AS unbilledScheduledStopAmount,
    SUM(b.dtm_bt_20_and_50_amount) AS dtmBt20And50Amount,
    SUM(b.standby_hour_amount) AS standbyHourAmount,
    ...
    SUM(b.commercially_stopped_amount) AS commerciallyStoppedAmount,
    SUM(b.mobilization_out_amount) AS mobilizationOutAmount,
    SUM(total) AS total
    FROM
      billings b
    JOIN
